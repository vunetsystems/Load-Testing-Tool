...
const users = usersRaw.map(line => {
    ...
    return {
        ...
        dashboardName: "Report Generation"  // Set dashboard name here
    };
...
// Metrics
const reportExecutionResponseTime = new Trend('report_execution_response_time');
const reportExecutionSuccessRate = new Rate('report_execution_success_rate');

export default function () {
    users.forEach(user => {
        let url = 'https://164.52.213.158/vuSmartMaps/api/1/bu/1/report_template/3/';
        ...
        reportExecutionResponseTime.add(res.timings.duration, { dashboardName: user.dashboardName });
        reportExecutionSuccessRate.add(res.status === 200, { dashboardName: user.dashboardName });
