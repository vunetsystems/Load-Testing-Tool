...
# ----------------------
[ -f "$LOGIN_CSV" ] || echo "timestamp,test_name,avg_response_time,status_code,success_rate,vus,vus_max,duration,segment_number" > "$LOGIN_CSV"
[ -f "$DASHBOARD_CSV" ] || echo "timestamp,dashboard_name,dashboard_avg_response_time,panel_id,panel_name,dashboard_status,dashboard_success_rate,panel_status,panel_success_rate,panel_avg_response_time,time_range,vus,vus_max,duration,segment_number" > "$DASHBOARD_CSV"
[ -f "$SUMMARY_FILE" ] || echo -e "DASHBOARD PERFORMANCE SUMMARY\n" > "$SUMMARY_FILE"

# ----------------------
# Ensure ClickHouse tables have the segment_number column
# ----------------------
echo "Ensuring ClickHouse tables have 'segment_number' column..."
...
run_k6() {
  ...
  if [[ "$script_name" == "login.js" ]]; then
    ...
    ( K6_INSECURE_SKIP_TLS_VERIFY=true k6 run \
      ...
      "$BASE_DIR/login.js" ) 2>&1 | tee "$out" || echo "⚠️  k6 login run finished with non-zero status (threshold failure?), continuing..."
  else
    ( K6_INSECURE_SKIP_TLS_VERIFY=true k6 run \
      ...
      --vus "$vus" \
      --duration "${duration_sec}s" \
      "$BASE_DIR/multi_dashboard_test.js" ) 2>&1 | tee "$out" || echo "⚠️  k6 dashboard run finished with non-zero status (threshold failure?), continuing..."
  fi
  ...
  if [[ "$script_name" == "login.js" ]]; then
    parse_and_insert_login "$out" "$vus" "${duration_sec}s" "$segnum"
  else
    parse_and_insert_dashboard "$out" "$time_range" "$vus" "${duration_sec}s" "$segnum"
  fi
}
...
parse_and_insert_login() {
  ...
  echo "timestamp,test_name,avg_response_time,status_code,success_rate,vus,vus_max,duration,segment_number" > "$temp_csv"
  ...
  grep -E "Login (successful|failed)" "$login_output" | while IFS= read -r line; do
    ...
    row="$timestamp,login,$RESPONSE_TIME,$STATUS,$SUCCESS_RATE,$vus,$vus,$duration_str,$segnum"
    echo "$row" >> "$temp_csv"
  ...
  if [ -s "$temp_csv" ]; then
    ...
    tail -n +2 "$temp_csv" | \
      ...
      -q "INSERT INTO monitoring.k6_login (timestamp,test_name,avg_response_time,status_code,success_rate,vus,vus_max,duration,segment_number) FORMAT CSV"
  fi
  rm -f "$temp_csv"
}
...
parse_and_insert_dashboard() {
  ...
  echo "timestamp,dashboard_name,dashboard_avg_response_time,panel_id,panel_name,dashboard_status,dashboard_success_rate,panel_status,panel_success_rate,panel_avg_response_time,time_range,vus,vus_max,duration,segment_number" > "$temp_csv"

  local row=""

  # Read the log file line by line to capture success AND failures
  while IFS= read -r line; do
    ...
    if [[ "$line" == *"DASHBOARD_DATA:"* ]]; then
      ...
      row="$timestamp,$dashboard_name,$response_time,,,'$status',$success_rate,,,$response_time,$time_range,$vus,$vus,$duration_str,$segnum"
      echo "$row" >> "$temp_csv"
      echo "$row" >> "$DASHBOARD_CSV"
      echo "🔹 Dashboard: $dashboard_name — ${response_time}ms (status=$status)" >> "$SUMMARY_FILE"

    # --- Dashboard Failure (ASSUMED FORMAT) ---
    elif [[ "$line" =~ ❌[[:space:]]Dashboard[[:space:]]failed[[:space:]]\|[[:space:]]name=([^|]+)[[:space:]]\|[[:space:]]status=([0-9]+)[[:space:]]\|[[:space:]]response_time=([0-9.]+)ms ]]; then
      dashboard_name="${BASH_REMATCH[1]}"
      status="${BASH_REMATCH[2]}"
      response_time="${BASH_REMATCH[3]}"
      success_rate=0
      ...
      echo "$row" >> "$temp_csv"
      echo "$row" >> "$DASHBOARD_CSV"
      echo "❌ Dashboard FAILED: $dashboard_name — ${response_time}ms (status=$status)" >> "$SUMMARY_FILE"
    ...
    # --- Panel Success ---
      ...
      row=$(printf "%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s" \
        "$timestamp" "$dashboard_name" "0" "$panel_id" "$panel_name" \
        "$status" "100" "$status" "100" "$response_time" "$time_range" "$vus" "$vus" "$duration_str" "$segnum")
      
      echo "$row" >> "$temp_csv"
      echo "$row" >> "$DASHBOARD_CSV"
      printf "  - Panel %-3s %-40s: %6.2fms (status: %s)\n" "$panel_id" "$panel_name" "$response_time" "$status" >> "$SUMMARY_FILE"

    # --- Panel Failure (ASSUMED FORMAT) ---
    elif [[ "$line" =~ ❌[[:space:]]Panel[[:space:]]failed[[:space:]]\|[[:space:]]dashboard=([^|]+)[[:space:]]\|[[:space:]]panel_id=([0-9]+)[[:space:]]\|[[:space:]]panel_name=([^|]+)[[:space:]]\|[[:space:]]status=([0-9]+)[[:space:]]\|[[:space:]]response_time=([0-9.]+)ms ]]; then
      dashboard_name="${BASH_REMATCH[1]}"
      panel_id="${BASH_REMATCH[2]}"
      panel_name="${BASH_REMATCH[3]}"
      status="${BASH_REMATCH[4]}"
      response_time="${BASH_REMATCH[5]}"

      # dashboard_avg_response_time = 0, dashboard_status=0, dashboard_success_rate=0
      # panel_status = $status, panel_success_rate=0
      row=$(printf "%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s" \
        "$timestamp" "$dashboard_name" "0" "$panel_id" "$panel_name" \
        "0" "0" "$status" "0" "$response_time" "$time_range" "$vus" "$vus" "$duration_str" "$segnum")

      echo "$row" >> "$temp_csv"
      echo "$row" >> "$DASHBOARD_CSV"
      printf "  - ❌ Panel FAILED %-3s %-35s: %6.2fms (status: %s)\n" "$panel_id" "$panel_name" "$response_time" "$status" >> "$SUMMARY_FILE"
  ...
  if [ -s "$temp_csv" ]; then
    ...
    tail -n +2 "$temp_csv" | \
      kubectl exec -i "$CLICKHOUSE_POD" -n "$CLICKHOUSE_NS" -- \
      clickhouse-client -d "$CLICKHOUSE_DB" --user "$CLICKHOUSE_USER" --password "$CLICKHOUSE_PASS" \
      -q "INSERT INTO monitoring.k6_results (timestamp,dashboard_name,dashboard_avg_response_time,panel_id,panel_name,dashboard_status,dashboard_success_rate,panel_status,panel_success_rate,panel_avg_response_time,time_range,vus,vus_max,duration,segment_number) FORMAT CSV"
  fi
  rm -f "$temp_csv"
}
