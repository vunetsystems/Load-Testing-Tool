...
def get_user_access_token(username, password):
    ...
    if response.status_code == 200:
        ...
        print(f"Failed to get access token for {username}: {response.status_code} - {response.text}")
        return None
...
def main(num_users):
    ...
    with open("user_cookies.txt", "a") as file:
        for _ in range(num_users):
            ...
            user_access_token = get_user_access_token(username, COMMON_PASSWORD)
            cookies = extract_cookies(username, COMMON_PASSWORD)
            if cookies and user_access_token:
                ...
                file.write(f"{username},{COMMON_PASSWORD},{user_access_token},{vunet_session},{X_VuNet_HTTP_Info},{grafana_session_expiry}\n")
