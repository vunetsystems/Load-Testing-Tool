...
type TestRun struct {
	...
	LagPerSource          sql.NullString  `json:"lag_per_source,omitempty"` // JSON: {"source1": {"min": 0.0, "avg": 1.5, "max": 5.0}, "source2": {...}}
	...
	TopicMetricsJSON      string          `json:"topic_metrics_json,omitempty"`   // JSON string for detailed per-topic avg/min/max metrics
}
